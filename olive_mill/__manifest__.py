# Copyright 2018-2023 Barroux Abbey (https://www.barroux.org/)
# @author: Alexis de Lattre <alexis.delattre@akretion.com>
# License AGPL-3.0 or later (http://www.gnu.org/licenses/agpl).

{
    'name': 'Olive Mill',
    'version': "18.0.1.0.0",
    'category': 'Manufacturing',
    'license': 'AGPL-3',
    'summary': 'Manage an olive mill',
    'author': 'Akretion,Barroux Abbey',
    'website': 'https://github.com/akretion/vertical-olive-mill',
    'depends': [
        'mrp',
        'account',
        'product_expiry_simple',  # OCA/stock-logistics-workflow
        'stock_no_negative',  # OCA/stock-logistics-workflow
        # 'stock_pack_operation_auto_fill',
        'report_py3o',  # OCA/repoting-engine
        'base_usability_akretion',  # akretion/odoo-usability, for reports
        'base_view_inheritance_extension',  # OCA/server-tools
        'product_detailed_type_stock',  # akretion/odoo-usability
        # 'web_view_calendar_list',  # OCA/web  not migrated to 18.0 yet
        ],
    'data': [
        'security/olive_security.xml',
        'security/ir.model.access.csv',
        'data/decimal_precision.xml',
        'data/sequence.xml',
        'data/organic_certifying_entity.xml',
        'data/protected_geo_indication.xml',
        'report/report.xml',
        'views/menu.xml',
        'wizards/olive_palox_case_lend_view.xml',
        'wizards/olive_palox_generate_production_view.xml',
        'wizards/olive_withdrawal_view.xml',
        'wizards/olive_invoice_create_view.xml',
        'wizards/olive_oil_production_ratio2force_view.xml',
        'wizards/olive_oil_production_force_ratio_view.xml',
        'wizards/olive_oil_production_pack2check_view.xml',
        'wizards/olive_oil_tank_transfer.xml',
        'wizards/olive_arrival_warning_view.xml',
        'wizards/olive_oil_production_product_swap_view.xml',
        'wizards/olive_oil_bottling_view.xml',
        'wizards/olive_oil_picking_view.xml',
        'wizards/olive_appointment_print_view.xml',
        'wizards/olive_oil_production_day_print_view.xml',
        'wizards/olive_partner_warning_print_view.xml',
        'wizards/res_config_settings_view.xml',
        'wizards/olive_oil_stock_report_view.xml',
        'wizards/olive_oil_sale_report_view.xml',
        'views/stock_location.xml',
        'views/stock_warehouse.xml',
        'views/stock_picking.xml',
        'views/olive_variant.xml',
        'views/olive_ochard.xml',
        'views/olive_parcel.xml',
        'views/olive_palox.xml',
        'views/olive_season.xml',
        'views/olive_lended_case.xml',
        'views/olive_cultivation.xml',
        'views/olive_treatment.xml',
        'views/olive_arrival.xml',
        'views/olive_oil_production.xml',
        'views/partner_organic_certification.xml',
        'views/olive_appointment.xml',
        'views/olive_preseason_poll.xml',
        'views/olive_sale_pricelist.xml',
        'views/organic_certifying_entity.xml',
        'views/res_partner.xml',
        'views/res_users.xml',
        'views/product.xml',
        'views/protected_geo_indication.xml',
        'views/stock_lot.xml',
        'views/olive_oil_analysis.xml',
        'views/mrp_bom.xml',
    ],
    'demo': [
        'demo/product.xml',
        'demo/protected_geo_indication.xml',
        'demo/mrp_bom.xml',
        'demo/olive_variant.xml',
        'demo/res_company.xml',
        'demo/olive_sale_pricelist.xml',
        'demo/res_partner.xml',
        'demo/olive_season.xml',
        'demo/stock_lot.xml',
        'demo/stock_location.xml',
        'demo/stock_warehouse.xml',
        'demo/olive_ochard.xml',
        'demo/olive_treatment.xml',
        'demo/olive_parcel.xml',
        'demo/olive_palox.xml',
        ],
    'installable': True,
    'application': True,
}
