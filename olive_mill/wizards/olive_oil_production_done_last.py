...
class OliveOilProductionDoneLast(models.TransientModel):
    ...
    warehouse_id = fields.Many2one(related='last_production_id.warehouse_id')
    compensation_location_id = fields.Many2one(
        related='last_production_id.compensation_location_id')
    compensation_oil_product_id = fields.Many2one(
        related='last_production_id.compensation_location_id.oil_product_id')
    season_id = fields.Many2one(related='last_production_id.season_id')
    company_id = fields.Many2one(related='last_production_id.company_id')
    next_first_production_id = fields.Many2one(
        'olive.oil.production', string='Next First-of-day Production')
    ...
    def validate(self):
        ...
        self.next_first_production_id.write({
            'compensation_sale_location_id': self.next_first_compensation_sale_location_id.id})
