            ...
            if invoice.partner_id.olive_farmer:  # just for perf
                if invoice.type == 'out_invoice':
                    oalines = self.env['olive.arrival.line'].search([
                        ('out_invoice_id', '=', invoice.id)])
                    if oalines:
                        company = invoice.company_id
                        oproducts = self.env['product.product'].search([
                            ('olive_type', '=', 'service')])
                        production_service = company.olive_oil_production_product_id
                        total_amount = 0.0
                        total_qty = 0.0
                    ...
                    for iline in invoice.invoice_line_ids:
                        ...
                        if oalines:
                            ...
                            if iline.uom_id != liter_uom:
                                raise UserError(_(
                                    "Invoice Line '%s' has an olive oil "
                                    "product so it should have liter as "
                                    "unit of measure.") % iline.name)
