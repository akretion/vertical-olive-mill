...
class OliveParcel(models.Model):
    ...
    partner_id = fields.Many2one(
        related='ochard_id.partner_id', string='Farmer', index=True, store=True)
    land_registry_ref = fields.Char(string='Land Registry Ref')
    area = fields.Float(
        string='Area', digits='Olive Parcel Area', help="Area in hectare")
    ...
    variant_label = fields.Char(
        compute='_compute_variant_label', store=True)
    ...
    cultivation_method = fields.Char()
